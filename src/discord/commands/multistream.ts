...
async function handleCommand(interaction: APIApplicationCommandInteraction, env: Env, ctx: ExecutionContext) {
  ctx.waitUntil(updateInteraction(interaction, env, { content: `${await findBotCommandMarkdown(env, 'multistream')} commands have moved to ${await findBotCommandMarkdown(env, 'streams', 'multistream')}` }))
