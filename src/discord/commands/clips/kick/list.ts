...
export const CLIPS_KICK_LIST_COMMAND = {
  type: 1,
  name: 'list',
  description: '(BETA) View your subscribed KICK clip channels',
  dm_permission: false,
}

export async function handleClipsKickListCommand(interaction: APIApplicationCommandInteraction, command: APIApplicationCommandInteractionDataSubcommandOption, env: Env) {
  ...
  return await updateInteraction(interaction, env, { embeds: [buildSuccessEmbed(clipsList, env, { title: `${KICK_EMOTE.formatted} Clip Notifications` })] })
}
