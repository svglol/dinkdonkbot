...
export const CLIPS_KICK_EDIT_COMMAND = {
  type: 1,
  name: 'edit',
  description: '(BETA) Update the settings for a Kick clip subscription',
  dm_permission: false,
  options: [{
    type: 3,
    name: 'streamer',
    description: 'The name of the Kick streamer to update',
    required: true,
    autocomplete: true,
  }, {
    type: 7,
...
export async function handleClipsKickEditCommand(interaction: APIApplicationCommandInteraction, command: APIApplicationCommandInteractionDataSubcommandOption, env: Env) {
  ...
  if (!streamer)
    return await updateInteraction(interaction, env, { embeds: [buildErrorEmbed('Invalid arguments', env)] })
  const dbClip = await useDB(env).query.kickClips.findFirst({
  ...
  if (channel) {
    ...
    await useDB(env).update(tables.kickClips).set({ channelId: channelValue }).where(and(like(tables.kickClips.streamer, streamer), eq(tables.kickClips.guildId, server)))
  }

  return await updateInteraction(interaction, env, { embeds: [buildSuccessEmbed(`Edited \`${streamer}\` for clip notifications`, env)] })
}

export async function handleClipsKickDBAutoComplete(interaction: APIApplicationCommandAutocompleteInteraction, option: APIApplicationCommandInteractionDataOption<InteractionType.ApplicationCommandAutocomplete>, env: Env) {
  if (option.type === ApplicationCommandOptionType.Subcommand) {
    ...
    // we can auto complete the streamer field
      ...
      const cacheKey = `autocomplete:${interaction.guild_id}:clips:kick:${option.name}:${input}`
      ...
      const streamers = await useDB(env).query.kickClips.findMany({
