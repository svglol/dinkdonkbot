...
async function handleCommand(interaction: APIApplicationCommandInteraction, env: Env, ctx: ExecutionContext) {
  ctx.waitUntil(updateInteraction(interaction, env, { content: `${await findBotCommandMarkdown(env, 'twitch')} commands have moved to ${await findBotCommandMarkdown(env, 'stream', 'twitch')}` }))
