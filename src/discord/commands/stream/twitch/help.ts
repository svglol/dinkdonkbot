...
export const TWITCH_HELP_COMMAND = {
  type: 1,
  name: 'help',
...
export async function handleTwitchHelpCommand(interaction: APIApplicationCommandInteraction, command: APIApplicationCommandInteractionDataSubcommandOption, env: Env) {
  const helpCard = {
    type: 17,
...
- ${await findBotCommandMarkdown(env, 'stream', 'twitch', 'list')} - List the Twitch streamers that you are subscribed to  
...
- ${await findBotCommandMarkdown(env, 'stream', 'twitch', 'help')} - Get this help message  
